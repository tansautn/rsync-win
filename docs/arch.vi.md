  ...
  Bước 2 — Convert identity path sang Unix (quan trọng):
  ...
  KHÔNG convert:
      ...
      path_rsync() (path đến rsync.exe) ← PathBuf, Windows format

  Bất nhất thú vị: ssh.exe path trong -e argument vẫn là Windows path (vì dùng PathBuf::display() trên Windows → backslash), nhưng identity path lại là Cygwin path. Điều này hoạt động được vì Cygwin
  runtime (cygwin1.dll) có thể tự handle cả hai format khi được spawn từ Windows process.
  ...
  → rsync-win.exe và cygwin64/ phải nằm cùng thư mục. Nếu tách ra, tool sẽ fail.
  ...
  │
  ├── path_win_to_unix(src)   ← cygpath -u (bypass nếu là SSH path)
